...
if IS_MAXWELL_GPU:
    # M40 / Maxwell compatibility: no Triton or FP8 support (needs sm_70+ / sm_89+)
    # Provide minimal stubs so that the rest of Unsloth can import these names.

    def weight_dequant_block(
        x: torch.Tensor, s: torch.Tensor, block_size: int = 128, dtype = torch.bfloat16
    ) -> torch.Tensor:
        # Simple block dequant fallback without Triton
        return x.to(dtype)
    ...
    def act_quant(x: torch.Tensor, block_size: int = 128):
        raise NotImplementedError("FP8 act_quant not supported on Maxwell GPU (needs sm_89+)")

    def fp8_linear(X, weight, weight_scale, bias = None):
        W_deq = weight_dequant(weight, weight_scale, dtype = X.dtype)
        out = torch_matmul(X, W_deq.t())
        if bias is not None:
            out = out + bias
        return out

    def fp8_torch_block_quant_forward(X, weight, weight_scale):
        return fp8_linear(X, weight, weight_scale)

    def fbgemm_fp8_linear(X, weight, weight_scale, bias = None):
        return fp8_linear(X, weight, weight_scale, bias)

    def fp8_fbgemm_block_linear(X, weight, weight_scale, bias = None):
        return fp8_linear(X, weight, weight_scale, bias)

    fp8_block_matmul = None
    ...
    # --- Original Triton-based implementation (sm_70+) ---
